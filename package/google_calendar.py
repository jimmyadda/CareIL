...
def sync_appointment_event(client_key, app_id):
    ...
    try:
        ...
        body = {
            # CareIL's calendar already identifies the client. Keep Google in sync
            # with the same useful title, without relying on a Railway flag.
            'summary': ('Therapy appointment – ' + client_name) if client_name else 'Therapy appointment',
